...
{
public:
    TensorPrint(std::ostream &o, const nn::Tensor<TTensor> &t);
...
protected:
    ...
    std::ostream &out_;
    const nn::Tensor<TTensor> &tensor_;
    const nn::Storage<typename TTensor::Storage> storage_cache_;
    const std::vector<long> stride_cache_;
    const std::vector<long> size_cache_;
    const typename TTensor::Storage::StorageBase * const data_;
    const int offset_;
};


template <class TTensor>
TensorPrint<TTensor>::TensorPrint(std::ostream &o, const nn::Tensor<TTensor> &t) : out_(o), tensor_(t),
...
void TensorPrint<TTensor>::getPrintFormat(int offset, double &scale, int &len)
{
    scale = 1.0f;
    bool is_int = true;
    auto minval = std::numeric_limits<typename TTensor::Storage::StorageBase>::max();
    auto maxval = std::numeric_limits<typename TTensor::Storage::StorageBase>::min();
